...
void MapComponent::init()
{
	...
	sound = g_audio_manager->CreateSound("event:/talk_se");
...
struct EditorData
{
	...
	void add_trigger()
	{
		...
		char out[10240];
		sprintf_s(out, out1, trigger_size[0], trigger_size[1], event_name, script[0], script[1], script[2], script[3], script[4], script[5], position.x, position.y, rotation.x);
...
void MapComponent::update(f32 dt)
{
	...
	case GameState::E_UI:
	{
		...
		{
			if(old_ms==0) g_audio_manager->Play("event:/talk_se");
	...
	case GameState::E_TALK:
	{
		if (cur_npc_ui)
		{
			...
			g_temp_uisys->begin();
			g_temp_uisys->clear();
			if (cur_npc_ui->word_tp == NPCDisplayData::WordType::E_SIMPLE_TALK)
			{
				
				g_temp_uisys->draw_box(80, 35, 600, 40, RBColorf(0.f, 0.f, 0.f, 0.5f));
				if (cur_npc_ui->face)
			...
			{
				int wlen = g_temp_uisys->get_text_len(words1);
				RBVector2I sp = cam->world_to_screen(RBVector2(cur_npc_ui->npc_size.x, cur_npc_ui->npc_size.y));
				int woff = 0;
				int sx = sp.x - wlen*0.5f;
				if (sx < 0)
					woff -= sx;
				if(sx+ wlen + 40>cam->viewport->w)
					woff += (cam->viewport->w - sx - wlen - 40);
				g_temp_uisys->draw_box(sp.x - wlen*0.5f+woff, sp.y + cur_npc_ui->npc_size.z, wlen+40, 35, RBColorf(0.f, 0.f, 0.f, 0.8f));
				g_temp_uisys->draw_text(sp.x+20-wlen*0.5f+woff, sp.y+10+cur_npc_ui->npc_size.z, words1, wcslen(words1), wlen+20);
			}
			g_temp_uisys->end();
		}
	}
	...
	if (game_state == GameState::E_PLAYER_CONTROLL)
	{
		if (d.size() > 0.1f)
		{
			d.normalize();
	...
	if (edit_mode)
	{
		ImGui::Begin("Game Varible");
		auto it = g_game_data->game_varible.begin();

		for (;it!= g_game_data->game_varible.end();++it)
		{
			Game_Varible::VarType tp = it->second.tp;
			switch (tp)
			{
			case Game_Varible::E_TEXT:
			{
				char ss[100];
				memcpy(ss, it->second.text.data(), it->second.text.size());
				ImGui::InputText(it->first.data(), ss, 100);
				it->second.text = ss;
			}
				break;
			case Game_Varible::E_INT:
			{
				ImGui::InputInt(it->first.data(), &it->second.number.integer);
			}
				break;
			case Game_Varible::E_FLOAT:
			{
				ImGui::InputFloat(it->first.data(), &it->second.number.real);
			}
				break;
			default:
				break;
			}
			//ImGui::Checkbox(it->first.data(),);
		}
		ImGui::End();
	}
